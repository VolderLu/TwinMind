...
try {
  ...
  process.stderr.write(`tm-post-op: cannot read ${cfgPath}: ${e.message}\n`);
  process.exit(1);
}

const target = join(pluginRoot, 'scripts', 'post-op.mjs');
const child = spawn(process.execPath, [target, ...process.argv.slice(2)], { stdio: 'inherit' });
child.on('error', (e) => {
  process.stderr.write(`tm-post-op: spawn failed: ${e.message}\n`);
  process.exit(1);
});
