...
更新 `vault-index.json`：`standalone_actions` 新增條目（含 title/status/tasks_total/tasks_done），`stats.total_actions` += 1。執行 post-op（Bash tool，`node .claude/twinmind/bin/tm-post-op.mjs --layer action --event '...'`）（event_type: `ACTION_CREATED`，layer: action）。
...
執行 post-op（Bash tool，`node .claude/twinmind/bin/tm-post-op.mjs --layer action --event '...'`）（event_type: `ACTION_COMPLETED`，layer: action）。
...
流程：調用 `twinmind:project` 建專案（用 action 標題），將 action 內容移入新專案的 `actions.md` 作為第一個 action，刪除原始 action 檔案，從 index 的 `standalone_actions` 移除。執行 post-op（Bash tool，`node .claude/twinmind/bin/tm-post-op.mjs --layer action --event '...'`）（event_type: `ACTION_PROMOTED_TO_PROJECT`，layer: action）。
